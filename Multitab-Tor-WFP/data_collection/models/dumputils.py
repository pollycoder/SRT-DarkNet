...
class Sniffer(object):
    ...
    def start_capture(self, pcap_path=None, pcap_filter=""):
        ...
        command = 'dumpcap -P -i eth0 -s 0 -f \'{}\' -w {}'\
            .format(self.pcap_filter, self.pcap_file)
        ...
        print("INFO\tdumpcap started in {}".format(utils.cal_now_time()))
    ...
    def stop_capture(self):
        ...
        if os.path.isfile(self.pcap_file):
            log.wl_log.info('Dumpcap killed. Capture size: %s Bytes %s' %
                        (os.path.getsize(self.pcap_file), self.pcap_file))
        else:
            log.wl_log.warning('Dumpcap killed but cannot find capture file: %s'
                           % self.pcap_file)
